...
const Specializations = ({ value, onChange, hasError = false, errorId }) => {
    ...
    useEffect(() => {
        if (inputValue) {
            ...
            const filtered = SPECIALIZATION_SUGGESTIONS.filter(
                (specialization) =>
                    specialization.toLowerCase().includes(formattedInput.toLowerCase()) &&
    ...
    const handleInputKeyDown = (e) => {
        // Add specialization on Enter if not empty and not already in the list
        ...
        // Remove last specialization on Backspace if input is empty
    ...
    const removeSpecialization = (specialization) => {
        onChange(value.filter((i) => i !== specialization));
    };
    ...
    return (
        <div className="relative">
            ...
            >
                {value.map((specialization, index) => (
                ...
                <input
                    ...
                    placeholder={value.length === 0 ? "Add specializations with #..." : "Add more..."}
                />
            </div>
